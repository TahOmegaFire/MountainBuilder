...
int main(int argc, char* argv[])
{
	...
	SDL_Event e;
	bool endProgram, pressedW = false;
    ...
    while(!endProgram)
    {
        ...
        while(SDL_PollEvent(&e))
        {
			...
			case SDL_MOUSEBUTTONDOWN:
				if(curMap.m_Tiles[Tile.x][Tile.y].m_Type == Tile::EMPTY)
					curMap.m_Tiles[Tile.x][Tile.y].m_Type = Tile::TILE1;
				else
					curMap.m_Tiles[Tile.x][Tile.y].m_Type = Tile::EMPTY;
            }
        }
		...
		if(currentKeyStates[SDL_SCANCODE_1])
		{
			for(size_t i = 0; i < curMap.m_Width; i++)
			{
				...
				{
					curMap.m_Tiles[0][j].m_Type = Tile::TILE1;
					curMap.m_Tiles[i][0].m_Type = Tile::TILE1;
					curMap.m_Tiles[curMap.m_Width - 1][j].m_Type = Tile::TILE1;
					curMap.m_Tiles[i][curMap.m_Length - 1].m_Type = Tile::TILE1;
				}
			}
		}
